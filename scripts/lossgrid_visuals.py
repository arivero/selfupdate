...
def main():
    ...
    for run in sorted(RUNS.glob("a_lossgrid_*")):
        ...
        if wp.exists():
            ...
            ax.plot(prof.index, prof.values, marker="o", ms=3, lw=1.2)
            ax.set_xlabel("layer")
            ax.set_ylabel("RMS relative parameter modification")
            ax.set_title(f"{run.name} — modification by layer", fontsize=9)
            ax.grid(alpha=.25)
            fig.tight_layout()
            fig.savefig(run / "eval/weight_deltas.png", dpi=180)
            plt.close(fig)
