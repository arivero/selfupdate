...
CFG_DIR = ROOT / "configs" / "experiments" / "v5"
QUEUE = ROOT / "scripts" / "queue_v5_ladder_20260712.tsv"
...
run_name: v5_{tag}_base_never_trained
model:
  name: {name}
data:
  examples_path: data/combined/examples_v5_window.jsonl
mask:
  mode: rag_tool
  compaction: remove
train:
...
run_name: {run}
data:
  examples_path: data/combined/examples_v5_{scope}.jsonl
mask:
  compaction: {censor}
train:
  hidden_loss: {loss}
...
run_name: v5cache_{tag}_{scope}_{censor}
model:
  name: {name}
data:
  examples_path: data/combined/examples_v5_{scope}.jsonl
...
def main() -> None:
    ...
    for tag, name, train_mb, cache_mb, offload in MODELS:
        base = f"configs/experiments/v5/base_{tag}.yaml"
        ...
        for scope in SCOPES:
            for censor in CENSORS:
                cache_cfg = f"configs/experiments/v5/cache_{tag}_{scope}_{censor}.yaml"
        ...
        for ltag, loss, slide, extra in LOSSES:
            ...
            for scope in SCOPES:
                for censor in CENSORS:
                    run = f"v5_{tag}_{ltag}_{scope}_{censor}"
                    arm = f"configs/experiments/v5/{run}.yaml"
                    (ROOT / arm).write_text(ARM_TPL.format(
                        run=run, loss=loss, slide=slide, scope=scope,
                        censor=censor, jlens=jlens), encoding="utf-8")
    ...
    header = (
        "# v5 ladder campaign (2026-07-12): 3 best lossgrid losses x "
