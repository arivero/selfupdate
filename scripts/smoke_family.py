...
result (gpt-oss-20b's MoE layout is *expected* to fail loudly).

Usage:
    python scripts/smoke_family.py --model Qwen/Qwen3-0.6B      # control
    python scripts/smoke_family.py --all                        # 4 families
"""
...
from selfupdate.train.layerwise import local_block_step, tail_step
...
STAGES = ["load", "adapt", "teacher", "step", "tail", "gen"]
...
def smoke_one(name: str, examples: str, tail_blocks: int, max_new: int,
    ...
    try:
        ...
        assert all(p.grad is None for p in _frozen_params(stack)), \
            "gradient leaked into frozen vocab (tail)"
        model.zero_grad(set_to_none=True)
        row[stage] = "OK"
...
def main():
    ...
    ap.add_argument("--tail-blocks", type=int, default=2)
    ...
    for name in models:
        ...
        row = smoke_one(name, args.examples, args.tail_blocks, args.max_new)
        rows.append(row)
