# Pipeline-v4.6 runtime guide

This document describes the only active training runtime on this branch:
pipeline v4.6. The scientific protocol and full knob contract live in
...
Native stage-owned evaluation and cache provenance are documented in
...
For block `L`, pipeline v4.6 trains
...
co-located neighbors may use node-local `/dev/shm` for the asynchronous
fixed-sequence relay. Epoch batteries use a separate all-rank NCCL group and
never materialize adapter shards or a complete model. Gemma shared K/V is an
explicit side channel in both protocols; rotary owners page their own block
