...
The initial 27B GPU-cache trainer also OOMed during first-cohort prefill with
42.82 GiB allocated on GPU0 and 59 MiB free before a 68-MiB request.  Its
561-GiB cache `751b16cc912b8cae` is complete and retained.  The unique `cpu`
retry uses the same PP4 `[16,32,48]`, B256/K16, Huber loss, and teacher-hidden
trajectory while staging only active tiles from that cache.  The failed GPU
directory is diagnostic only.
