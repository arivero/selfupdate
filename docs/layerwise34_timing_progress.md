...
| 06:xx | 4B PP4 sole retry and 27B admission running | PP4 v2 `[7,15,23]` is the only repartition retry on agpul06; its corrected argv-pattern sampler is collecting a real occupancy trace. In parallel, Qwen3.6-27B starts at PP2—not PP1—on agpul04 physical GPUs 1/3. The 52 GiB model snapshot is staged in shared memory and the v3.2 launcher is regenerating cache identity `ef92bc2ccff8c62d` locally. The cache builder holds 36.8/36.6 GiB on GPUs1/3 with a 100%, ~300 W sample on GPU3 and no allocation on occupied GPUs0/2. |
