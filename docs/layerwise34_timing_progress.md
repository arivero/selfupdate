...
| 08:0x | Teacher-coordinate mismatch found; prior teacher-hidden speeds invalidated | The 0.8B full-input cache contains an exact 1,144-position generation prompt while its student flow-mask view has 1,024 positions. Locality exposed that the first implementation indexed teacher inputs with student `s0`/positions and censored teacher history with the student flow mask. The corrected law uses cache `t0`, monotonic teacher positions, and an uncensored teacher-prefix causal state; aligned losses slice at teacher `t0`. The diagnostic 0.8B/4B CPU and 4B GPU speeds remain engineering evidence only and must be repeated before acceptance. Ordinary student-hidden results are unaffected. |
| 08:2x | Final corrected small-model teacher-hidden runs accepted | With full inputs resident on each owning GPU and the ordinary partitions unchanged, 0.8B PP4 completed 49,453 events in 67.432 s (**733.38/s**) and 4B PP3 completed 11,020 in 19.777 s (**557.21/s**). Both report exactly zero boundary bytes, passed locality/frozen-vocabulary certification, completed exact whole-100 CE/KL evaluation, and published checkpoints. 0.8B CE/KL: 0.25048/0.0000555 over 42,999 answer tokens; 4B: 0.03641/0.0000180 over 4,566. Per-card reserved VRAM was 10.48/10.76/10.76/10.66 GiB and 13.46/13.33/11.87 GiB respectively. These exact-100 rates are not substituted for full-v5 ordinary rates; they establish the teacher-hidden mode itself. |
| 08:2x | 27B PP4 baseline frozen; sole optimization retry launched | Equal cuts `[16,32,48]` drained six cohorts before the cooperative stop: 149,902 events in 462.924 s (**323.82/s**) and 13.13 global physical writes/s. Locality passed and the checkpoint published; partial output evaluation covered 54,154/70,670 answer tokens and correctly published no CE/KL result. Reserved VRAM was 29.19/26.61/26.61/29.91 GiB. The sustained sampler averaged roughly 28--30% and 159--165 W per card despite 97--100% peaks. Stage compute was a minority of wall time, so the one retry keeps the partition fixed and pipelines four independent prompt-prefill shards. |
| 08:3x | 27B PP4 optimization rejected; teacher-hidden final launched | The sole optimization attempt failed before its first write when four host threads entered the same Triton autotuner concurrently (`TypeError: 'NoneType' object is not a mapping`). It therefore has no speed result and cannot displace the certified **323.82/s** baseline. The unsafe knob is now rejected at dispatch. The final 27B teacher-hidden arm keeps the identical ordinary `[16,32,48]` ownership on physical GPUs0/1/2/3; only each block's activation source changes to its immutable cached teacher input. |
| 08:4x | 27B teacher-hidden PP4 completed and certified | Cache `c742bd2a926bfbc5` supplied complete teacher-coordinate inputs resident on each owning GPU. The exact-100 traversal completed 9,836 token events in 159.052 s (**61.84/s**) and 1,600 physical writes (**10.06/s**) with exactly zero boundary bytes. All four 16-block stages completed 25 tiles; stage compute was 35.01/35.38/33.18/34.22 s. Whole-set output evaluation covered exactly 100 items/3,382 answer tokens (CE **0.0183121**, KL **0.0000070778**). Locality passed with zero cross-block and frozen-vocabulary leakage, and the checkpoint published. Peak allocated VRAM was 31.65/29.26/29.26/32.58 GiB. The sampler shows individual 99--100% peaks but only 22.84--25.81% average utilization and 130.08--134.47 W/card: the current independent dispatcher serializes stage work, so this is a valid negative saturation result rather than a no-communication scaling win. |
| Overnight | Full-v5 teacher-hidden matrix prepared for 12 GPUs | To remove the exact-100 packing confound, agpul04 receives PP2 0.8B Huber/cosine on GPUs01/23, agpul05 receives PP2 4B Huber/cosine on GPUs01/23, and agpul06 receives PP4 27B Huber on GPUs0123. Cuts remain `[12]`, `[16]`, and `[16,32,48]`; all use GPU-resident teacher inputs, zero-boundary independent execution, and `epochs: 1000000` so they continue until externally terminated. Exact configs, run paths, and tomorrow's cooperative-stop/analysis protocol are in `docs/layerwise34_overnight_handoff.md`. |
| 03:00 | Corrected open-ended matrix launched; first cache published | All five launchers resolve `epochs: 1000000`. The full-v5 0.8B teacher-input cache `633ce19cf18c5963` published 2,071 examples/24 layers in 219.5 s, after which both PP2 loss arms became live trainers with telemetry. The 4B and 27B builders remain active; their peer loss launchers wait on the atomic model/cache lease. No launch has logged a traceback or OOM. |
| 03:01 | Both 4B PP2 loss trainers live | The shared 4B full-input cache `e6659930d7736004` published 2,071 examples/32 layers after 380.6 s. Cosine PID `451823` now owns GPUs2/3 and Huber PID `451896` owns GPUs0/1; both samplers are attached. The visible idle interval on GPUs2/3 was the required cache-lease wait. |
| 03:0x | 0.8B shard-64 overnight arms OOM; shard-32 retries prepared | Both losses failed independently before an accepted epoch at the same footprint: 43.44 GiB allocated on GPU1/GPU3 and roughly 108 MiB free before a 130-MiB request. They were not stopped by the agent. Clean retry identities retain B256/K16, PP2 `[12]`, GPU pairs 01/23, and all scientific semantics while reducing only activation/prefill shard width from 64 to 32. |
| 03:0x | Shard-32 repeated identical OOM; GPU-cache residency defect isolated | Both shard-32 arms again reached 43.44 GiB, proving shard width did not control the dominant allocation. `_bk_prepare_cohort_shards` retains every GPU-cached teacher-input tensor from every shard for the whole cohort; halving width doubles shard count and leaves total residency unchanged. Final `cpu_cache` retries remain teacher-hidden and preserve B256/K16/PP2/loss/write semantics while explicitly changing only source residency to pinned-host tile staging. |
| 03:1x | 27B PP4 repeated GPU-cache residency OOM; CPU-cache retry prepared | The 561-GiB full cache `751b16cc912b8cae` published 2,071 examples/64 layers in 649.6 s. The first GPU-cache cohort then filled GPU0 to 42.82 GiB allocated (59 MiB free) and failed a 68-MiB prefill request. This is the same all-shards-resident defect as 0.8B. A unique CPU-cache PP4 retry preserves `[16,32,48]`, B256/K16, Huber, teacher coordinates, and writes while staging only active tiles. |
| 03:4x | Defective infinite runs stopped; target-reuse refactor under gate | All five trainers and samplers on agpul04/05/06 were terminated and all 12 GPUs verified idle; no CPU-cache workaround is being accepted as the final overnight implementation. The refactor now stages `h[L]` once for block L's loss and reuses it detached as `i[L+1]`, caches only the active BxK owned-stage range on each GPU, packs prompt inputs one layer at a time, and discards unused teacher-prefill outputs. A startup exact-cache-chain tripwire, config audit, numerical 100-question comparison, locality, and a full-v5 bounded-memory admission are required before any infinite relaunch. |
| 04:1x | Target-reuse numerical/locality comparison passed | The corrected 0.8B PP4 run drained 260 tiles/49,453 events at **837.38 token-events/s**, versus 733.38/s for the accepted pre-refactor placement. Cached `h[L] == i[L+1]` passed exactly over all 23 boundaries before training. CE/KL, every per-layer loss, and every per-layer gradient norm were exactly identical; checkpoint relative-L2 drift was **1.57e-13** with cosine **1.0**. Locality and frozen-vocabulary leakage remained exactly zero. Aggregate reserved VRAM fell from **42.66 to 24.25 GiB** (per card 5.01/5.70/5.27/8.27 GiB). Fresh open-ended configs are pinned, but no infinite run is authorized until the one-epoch full-v5 PP2 admission drains and saves. |
| 04:2x | Overnight acceptance conditions made explicit | For the teacher-hidden all-night matrix, utilization, watts, and temperature are telemetry rather than launch conditions. The explicit checks are committed/audited source, exact cache-chain and numerical/locality equivalence, a real B256 full-v5 cohort below the old OOM footprint, and completion/save of the bounded full-v5 epoch. The earlier saturation condition remains relevant only to the optional Pareto/InfiniBand extension and is waived for this matrix. |
| 04:2x | Full-v5 target-reuse admission passed | The clean-source 0.8B PP2 `[12]` Huber run completed all 2,071 items in nine B256 cohorts, evaluated exactly 947,644 answer tokens (CE **0.254836**, KL **0.00005747**), and measured **2,682.69 token-events/s**. Locality passed with zero cross-block and frozen-vocabulary leakage; `done` and the checkpoint published. Peak reserved VRAM was **13.65/25.39 GiB**, replacing the old invariant 43.44-GiB OOM. The five fresh `epochs: 1000000` target-reuse launches are therefore authorized. |
| 04:3x | Five corrected open-ended trainers placed on all 12 L40S cards | agpul04 runs 0.8B PP2 Huber/cosine on GPUs01/23; agpul05 runs 4B PP2 Huber/cosine on GPUs01/23; agpul06 runs 27B PP4 Huber on GPUs0123. The four small-model streams have clean commit `356ce29` contracts, exact cache-chain success, and correct physical mappings; 27B is still in epoch-zero/model initialization before its contract at this snapshot. One GPU sampler per host and one process sampler per trainer record the requested utilization/power and CPU/thread/fault/I/O evidence. The first cosine command on each two-run host incorrectly narrowed `CUDA_VISIBLE_DEVICES` and was rejected before model construction or metrics; corrected physical-ID launches are now the sole metric writers, and the failed command remains visible in the exact logs. |
| 04:3x | PP4 oscillation localized to internal NUMA/producer contention | The first 27B cohort completed with balanced stage compute (21.96/20.78/20.52/20.78 s), yet the 120-second GPU trace averaged only 19--22% and almost never had two cards above 50%. The node itself was 95--98% idle with ample RAM, zero I/O wait, zero major faults, and no competing GPU process. Trainer memory was severely NUMA-skewed while the main producer packed shared full-depth pinned target tiles on whichever CPU ran it; all cards occupy distinct NUMA nodes with SYS routes. This is evidence for stage-local NUMA-pinned packing in the next implementation, not a reason to mutate the live overnight streams. |
| 04:4x | All five streams clean and producing durable telemetry | Every run now reports commit `356ce29`, `runtime_dirty=false`, exact teacher-cache-chain success, correct card mapping, and owner-local BxK target residency. First completed-epoch rates are 0.8B Huber **2,840.45/s**, 0.8B cosine **2,824.08/s**, 4B Huber **1,132.21/s** (epoch 2: **1,241.05/s**), and 4B cosine **1,141.55/s**. Their exact whole-set CE/KL rows are present. 27B has drained three cohorts with no error but has not completed a full epoch, so no epoch rate is inferred. All trainers and samplers remain live for external termination. |
...
| Qwen3.6-27B | 4 | wavefront | `[16,32,48]`, GPUs 0/1/2/3 | full-v5 cache `ef92bc2ccff8c62d` | baseline passed/stopped; locality certified; sole optimization rejected before first write | 323.82 baseline | 29.19 / 26.61 / 26.61 / 29.91 GiB reserved |
| Qwen3.5-0.8B | 4 | independent teacher input, GPU cache | `[7,14,21]`, GPUs 0/1/2/3 | 100q full-input cache `1e992e1638f30bee` | passed; locality and checkpoint certified | 733.38 | 10.48 / 10.76 / 10.76 / 10.66 GiB reserved |
| Qwen3.5-4B | 3 | independent teacher input, GPU cache | `[11,23]`, GPUs 1/2/3 | 100q full-input cache `fd80e3e399e9b809` | passed; locality and checkpoint certified | 557.21 | 13.46 / 13.33 / 11.87 GiB reserved |
| Qwen3.6-27B | 4 | independent teacher input, GPU cache | `[16,32,48]`, GPUs 0/1/2/3 | 100q full-input cache `c742bd2a926bfbc5` | passed; locality, exact CE/KL, and checkpoint certified | 61.84 | 31.65 / 29.26 / 29.26 / 32.58 GiB allocated |
