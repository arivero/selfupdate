...
| Overnight | Full-v5 teacher-hidden matrix prepared for 12 GPUs | To remove the exact-100 packing confound, agpul04 receives PP2 0.8B Huber/cosine on GPUs01/23, agpul05 receives PP2 4B Huber/cosine on GPUs01/23, and agpul06 receives PP4 27B Huber on GPUs0123. Cuts remain `[12]`, `[16]`, and `[16,32,48]`; all use GPU-resident teacher inputs, one complete epoch, and zero-boundary independent execution. Exact configs, run paths, and tomorrow's analysis protocol are in `docs/layerwise34_overnight_handoff.md`. |
