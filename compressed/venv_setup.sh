...
#
# Usage:
#   compressed/venv_setup.sh                     # build (idempotent-ish; see --force)
#   compressed/venv_setup.sh --force             # delete and rebuild
#   SELFUPDATE_VENV=/tmp/$USER/other compressed/venv_setup.sh
#
# Then run everything through the interpreter it prints, from the repo root:
#   /tmp/$USER/selfupdate-venv/bin/python compressed/train.py --config ... --experiment ...
#
...
echo
echo "done: $VENV"
echo "verify with: compressed/venv_check.sh"
echo "use with:    $VENV/bin/python compressed/train.py --config ... --experiment ..."
