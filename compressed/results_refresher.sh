...
refresh() {
    for run in runs/pareto_v2_*; do
        ...
        compressed/l40s_exec.sh compressed/report_v2.py "$run" >/dev/null 2>&1 || true
    done
    compressed/l40s_exec.sh compressed/group_reports_v2.py --group-by all >/dev/null 2>&1 || true
}
