...
while [[ "$#" -gt 0 ]]; do
  case "$1" in
    ...
    --node-cache-wait-seconds)
      ...
      ;;
    -h|--help)
      echo "usage: compressed/l40s_train_v3.sh [--config PATH] --experiment PATH [--node-cache-wait-seconds N]"
      exit 0
      ;;
    *)
...
"$ROOT/compressed/l40s_exec.sh" "$ROOT/compressed/build_teacher_cache.py" \
  "${COMMON[@]}" --coordinated-node-cache \
  --node-cache-wait-seconds "$WAIT_SECONDS"
"$ROOT/compressed/l40s_exec.sh" "$ROOT/compressed/train.py" "${COMMON[@]}"
"$ROOT/compressed/l40s_exec.sh" "$ROOT/compressed/report_v2.py" "$RUN_NAME"
