...
Usage:
    python compressed/train_certify.py --list
    python compressed/train_certify.py --variant summed_item --out certs/pre/summed_item.json
    python compressed/train_certify.py --all --out-dir certs/pre
    python compressed/train_certify.py --variant summed_item --reference certs/pre/summed_item.json
    python compressed/train_certify.py --all --reference-dir certs/pre --out-dir certs/post
...
def _ensure_subset(n: int = 16) -> None:
    ...
    if not root.exists():
        ...
        subprocess.run(
            [sys.executable, str(REPO / "compressed" / "build_teacher_cache.py"),
             "--experiment", str(exp)],
            check=True, cwd=REPO,
        )
        exp.unlink()
