...
run_one() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" "$PY" "$ROOT/compressed/benchmark_vllm_generation.py" --model "$model" --batch-sizes 64 --gpu-memory-utilization 0.85 --max-model-len 4096 "${graph[@]}" --prompt-format native --out "$out" >>"$LOG" 2>&1
