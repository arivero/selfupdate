...
# this layer. Build it with compressed/l40s_setup.sh through a delegated launch.
set -euo pipefail

if [[ "${1:-}" == "python" || "${1:-}" == "python3" ]]; then
  echo "usage: compressed/l40s_exec.sh <script.py> [args...]" >&2
...
[[ -d "$DEPS/transformers" && -d "$DEPS/peft" ]] || {
  echo "missing L40S dependency layer: run compressed/l40s_setup.sh" >&2
  exit 2
}
