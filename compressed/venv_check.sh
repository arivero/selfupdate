...
#
# Usage: compressed/venv_check.sh
...
if [[ ! -x "$PY" ]]; then
  ...
  echo "  build it with: compressed/venv_setup.sh" >&2
  exit 1
fi
...
try:
    ...
    failures.append(
        ...
        "requirements-optional.txt (compressed/venv_setup.sh does by default)")
print()
if failures:
    print("FAIL:")
    for f in failures:
...
"$PY" "$ROOT/compressed/shell_helpers.py" venv-import-check
