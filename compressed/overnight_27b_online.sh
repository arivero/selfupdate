...
TQDM_DISABLE=1 "$PY" compressed/build_teacher_cache.py --coordinated-node-cache \
  --config "$BASE" --experiment "$EXP" --index-only \
  > runs/h100_27b_index_build.log 2>&1
INDEX=$("$PY" "$ROOT/compressed/shell_helpers.py" \
...
compressed/launch_v4_stages.sh "$BASE" "$EXP" || { log "ABORT: launch refused"; exit 1; }
sleep 420
for k in 0 1 2 3; do
