...
#
# Usage:
#   compressed/launch_v4_stages.sh <base.yaml> <experiment.yaml>
#
# Each stage k:
...
# Merge shards afterwards with compressed/merge_v4_adapters.py.
...
$("$PY" "$ROOT/compressed/shell_helpers.py" \
...
# per node with: compressed/stage_hf_cache.sh --shm <org/model>
...
if [[ -f "$SHM_HF/.selfupdate-hf-stage-ready" ]]; then
  ...
  echo "  compressed/stage_hf_cache.sh --shm <org/model>"
fi
...
for ((k = 0; k < STAGES; k++)); do
  ...
  if [[ -z "$host" || "$host" == "local" || "$host" == "$(hostname -s)" ]]; then
    nohup setsid "$PY" "$ROOT/compressed/train.py" \
    ...
    # /tmp venv (build it there first: compressed/venv_setup.sh). HF_HOME is
    ...
    ssh -o BatchMode=yes "$host" \
      "cd '$ROOT' && nohup setsid env $STAGE_ENV \
       /tmp/\$USER/selfupdate-venv/bin/python compressed/train.py \
       --config '$BASE' --experiment '$EXP' --v4-stage $k \
...
if [[ ${#local_pids[@]} -gt 0 ]]; then
  ...
  nohup setsid "$ROOT/compressed/v4_stage_reaper.sh" "$LEASE.local" \
...
echo "merge:  $PY compressed/merge_v4_adapters.py runs/$RUN_NAME"
