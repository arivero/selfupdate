...
check_once() {
    ...
    ps -u "$USER" -o pid,ppid,etime,cmd \
        | rg 'defactorised/(train|evaluate|teacher_ceiling|destruct_eval|standard_destruction_eval)\.py|gpu_scheduler\.sh|results_refresher|hourly_evidence_check' \
        || true
    ...
    if [[ -x .venv/bin/python ]]; then
        run_step "summarize standard destruction" .venv/bin/python defactorised/summarize_standard_destruction.py
        run_step "build pipeline-v2 grouped reports" defactorised/l40s_exec.sh defactorised/group_reports_v2.py --group-by all
    else
