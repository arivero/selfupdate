...
run_one() {
  ...
  if [[ ! -f "$out/responses_bs64.jsonl" ]]; then
    run_isolated env CUDA_VISIBLE_DEVICES="$visible" "$PY" \
      "$ROOT/defactorised/benchmark_vllm_generation.py" \
  ...
  CUDA_VISIBLE_DEVICES="$visible" "${SELFUPDATE_VENV:-/tmp/$USER/selfupdate-venv}/bin/python" \
    defactorised/build_teacher_cache.py \
