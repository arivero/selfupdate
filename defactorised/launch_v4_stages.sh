...
#
# Usage:
#   defactorised/launch_v4_stages.sh <base.yaml> <experiment.yaml>
#
# Each stage k:
...
# Merge shards afterwards with defactorised/merge_v4_adapters.py.
...
$("$PY" "$ROOT/defactorised/shell_helpers.py" \
    v4-launch-info "$BASE" "$EXP")
EOF2
...
# per node with: defactorised/stage_hf_cache.sh --shm <org/model>
...
if [[ -f "$SHM_HF/.selfupdate-hf-stage-ready" ]]; then
  ...
  echo "  defactorised/stage_hf_cache.sh --shm <org/model>"
fi
...
for ((k = 0; k < STAGES; k++)); do
  ...
  if [[ -z "$host" || "$host" == "local" || "$host" == "$(hostname -s)" ]]; then
    nohup setsid "$PY" "$ROOT/defactorised/train.py" \
    ...
    # /tmp venv (build it there first: defactorised/venv_setup.sh). HF_HOME is
    ...
    ssh -o BatchMode=yes "$host" \
      "cd '$ROOT' && nohup setsid env $STAGE_ENV \
       /tmp/\$USER/selfupdate-venv/bin/python defactorised/train.py \
       --config '$BASE' --experiment '$EXP' --v4-stage $k \
...
if [[ ${#local_pids[@]} -gt 0 ]]; then
  ...
  nohup setsid "$ROOT/defactorised/v4_stage_reaper.sh" "$LEASE.local" \
...
echo "merge:  $PY defactorised/merge_v4_adapters.py runs/$RUN_NAME"
