...
#
# Usage:
#   defactorised/venv_setup.sh                     # build (idempotent-ish; see --force)
#   defactorised/venv_setup.sh --force             # delete and rebuild
#   SELFUPDATE_VENV=/tmp/$USER/other defactorised/venv_setup.sh
#
# Then run everything through the interpreter it prints, from the repo root:
#   /tmp/$USER/selfupdate-venv/bin/python defactorised/train.py --config ... --experiment ...
#
# There is deliberately NO `pip install -e .` here. Defactorised entry points
# carry the project code they need, so one venv can safely serve checkouts.
...
# training: standard evaluation does a module-level
...
echo
echo "done: $VENV"
echo "verify with: defactorised/venv_check.sh"
echo "use with:    $VENV/bin/python defactorised/train.py --config ... --experiment ..."
