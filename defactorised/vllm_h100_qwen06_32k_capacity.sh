...
run_one() {
  ...
  CUDA_VISIBLE_DEVICES=0 "$PY" "$ROOT/defactorised/benchmark_vllm_generation.py" --model Qwen/Qwen3-0.6B --batch-sizes "$bs" --max-num-seqs "$bs" --gpu-memory-utilization 0.85 --max-model-len 32768 "${graph[@]}" --out "$out" >>"$LOG" 2>&1
