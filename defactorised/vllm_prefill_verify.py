...
Run in the vLLM env (single-node, unchanged since Phase 1):
  CUDA_VISIBLE_DEVICES=0 ../venvs/vllm025/bin/python defactorised/vllm_prefill_verify.py \
...
Multi-node (owner 2026-07-20, mirrors defactorised/vllm_2node_smoke.py): driven by
...
the summary and writes --out.
  ...
  ../venvs/vllm025/bin/torchrun --nnodes 2 --node-rank 0 --nproc-per-node 4 \
    ...
    defactorised/vllm_prefill_verify.py --model <path> --responses <jsonl> \
