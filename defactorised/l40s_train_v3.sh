...
while [[ "$#" -gt 0 ]]; do
  case "$1" in
    ...
    --node-cache-wait-seconds)
      ...
      ;;
    -h|--help)
      echo "usage: defactorised/l40s_train_v3.sh [--config PATH] --experiment PATH [--node-cache-wait-seconds N]"
      exit 0
      ;;
    *)
...
"$ROOT/defactorised/l40s_exec.sh" "$ROOT/defactorised/build_teacher_cache.py" \
  "${COMMON[@]}" --coordinated-node-cache \
  --node-cache-wait-seconds "$WAIT_SECONDS"
"$ROOT/defactorised/l40s_exec.sh" "$ROOT/defactorised/train.py" "${COMMON[@]}"
"$ROOT/defactorised/l40s_exec.sh" "$ROOT/defactorised/report_v2.py" "$RUN_NAME"
