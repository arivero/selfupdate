...
# this layer. Build it with defactorised/l40s_setup.sh through a delegated launch.
set -euo pipefail

if [[ "${1:-}" == "python" || "${1:-}" == "python3" ]]; then
  echo "usage: defactorised/l40s_exec.sh <script.py> [args...]" >&2
...
[[ -d "$DEPS/transformers" && -d "$DEPS/peft" ]] || {
  echo "missing L40S dependency layer: run defactorised/l40s_setup.sh" >&2
  exit 2
}
...
export PYTHONPATH="$ROOT/runtime/l40s:$DEPS${PYTHONPATH:+:$PYTHONPATH}"
